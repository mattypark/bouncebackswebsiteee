...
/*
  BB-1 buy page — built for the fewest possible clicks.

  Land here from any BUY BB-1 entry point, flip Subscribe/One-time if you
  want, then click a pack. The pack tile IS the buy button: it links straight
  to Shopify checkout with that variant (and selling plan) already applied.
  No cart, no drawer, no add-to-cart step anywhere on the site.

  LEFT  — gallery + subscriber-club card
  RIGHT — social proof → title → purchase toggle → price → pack tiles → spec
*/
...
export default function ProductDetail({ pack }: { pack: Pack }) {
  const current = PACKS.find((p) => p.pack === pack) ?? PACKS[0];
    ...
    <section className="w-full bg-bb-paper pt-10 pb-24 md:pt-16 md:pb-32">
      <div className="mx-auto grid max-w-6xl gap-12 px-6 lg:grid-cols-2 lg:gap-16">
        ...
        <div>
          ...
          {/* Subscriber club card */}
          <div className="mt-6 rounded-2xl border border-bb-mid/40 bg-white p-6 md:p-8">
            <p className="sport-kicker text-bb-mid">Subscriber Exclusive</p>
            <p className="sport-display mt-2 text-2xl text-bb-deep md:text-3xl">
        ...
        <div>
          ...
          <h1 className="sport-display mt-3 text-6xl text-bb-deep md:text-7xl">
            BB-1 {current.label}
          </h1>
          ...
          {/* Purchase type toggle — sets what every pack tile below buys */}
          ...
          {/* Price line for the pack you landed on */}
          ...
          {/* ── Pack tiles = the buy button. One click out to checkout. ── */}
              ...
              return (
                <a
                  ...
                  className={`group relative flex items-center justify-between rounded-xl border p-4 transition-all ${
                    isCurrent
                      ? "border-bb-deep bg-white ring-1 ring-bb-deep"
                ...
                >
                  ...
                  <div className="ml-4 shrink-0 text-right">
                    ...
                    <p className="text-[10px] font-black tracking-[0.1em] text-bb-mid uppercase opacity-0 transition-opacity group-hover:opacity-100">
                      Buy now →
                    </p>
                  </div>
                </a>
